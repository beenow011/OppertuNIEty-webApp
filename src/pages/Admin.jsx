...
function Admin() {
  ...
  const handleApprove = (userAddress) => {
    axios
      .post(`http://localhost:3000/api/coordinator-auth/approve`, {
        userAddress,
      })
      .then(() => {
  ...
  const handleDisapprove = (userAddress) => {
    axios
      .post(`http://localhost:3000/api/coordinator-auth/disapprove`, {
        userAddress,
      })
      .then(() => {
        setUnapprovedUsers((prevUsers) =>
      ...
      <ul className="user-list space-y-6">
        {unapprovedUsers?.map((user) => (
          <li
            ...
            className="user-item bg-gray-800/60 p-6 rounded-lg flex justify-between items-center shadow-lg hover:bg-gray-800/80 transition-all duration-300 transform  border border-gray-700 hover:border-gray-600"
          >
